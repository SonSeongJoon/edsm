...
import {initLimitExcessForm, LimitExcessForm} from '../components/form/LimitExcessForm';
import {HomeForm, initHomeForm} from '../components/form/HomeForm';
import {HomeReportForm, initHomeReportForm} from '../components/form/HomeReportForm'; // New import
...
const initForms = {
	...
	재택근무신청서    : initHomeForm,
	재택근무보고서    : initHomeReportForm,
};
const Forms = {
	지출결의서      : ExpendForm,
	휴가계        : VacationForm,
...
export default function Write() {
	...
	useEffect(() => {
		if (initialProduct?.file === '재택근무신청서') {
			...
			setProduct(updatedInitialProduct);
			// 여기에서 updatedInitialProduct를 저장 또는 업데이트할 수 있음
		} else {
	...
	}, [initialProduct, product.file, currentDate, userDept, userName, userPhoneNum, userCorporation]);
	...
	const FormComponent = initialProduct?.file === '재택근무신청서' ? HomeReportForm : Forms[product.file] || ExpendForm;
				...
				<div className='w-full lg:w-3/4 flex-2'>
					...
					<div className='flex items-center mb-4'>
						...
						<select
							...
							value={initialProduct?.file === '재택근무신청서' ? '재택근무보고서' : product.file}
							onChange={handleChange}
						>
							{options && options.map((option, index) => <option key={index}>{option}</option>)}
						</select>
					</div>
						<FormComponent product={product} setProduct={setProduct} handleChange={handleChange}
						               userDept={userDept}/>
				</div>
