...
export default function Home() {
	...
	const getStatusCounts = (items) => items.reduce((acc, state) => {
			...
			{isAdmin && (
				<>
					...
					<div className="grid grid-cols-1 gap-4 mb-6 w-full max-w-md">
						<div className="flex flex-col items-center justify-center bg-white border border-gray-300 rounded-lg p-4 shadow-md cursor-pointer transition transform hover:scale-105"
						     onClick={() => navigate('/receive')}>
							<span className="text-lg font-bold text-indigo-500 mb-1">결재를 기다리고 있습니다</span>
							<span className="text-gray-600">{receivedStatusCounts['대기'] || 0}건</span>
						</div>
					</div>
				</>
			)}
