...
import {HomeReportItemInput} from "../html/HomeReportItemInput";

const initHomeReportForm = {
	file         : '재택근무보고서',
	title        : '',
	dept         : '',
	deel         : '',
	items        : [{title: '', amount: '', percent: ''}],
	agree        : [],
	agreeName    : [],
	agreeUid     : [],
	workPlace    : '',
...
const HomeReportForm = ({product, setProduct, handleChange}) => {
	useEffect(() => {
	...
	const addItem = () => {
		if(product.items.length < 8) {
			setProduct((prevProduct) => ({
				...prevProduct,
				items: [...prevProduct.items, {title: '', amount: '', percent: ''}],
			}));
		} else {
...
export {HomeReportForm, initHomeReportForm};
