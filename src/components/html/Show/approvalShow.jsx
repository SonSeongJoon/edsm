...
const ApprovalShow = ({ product }) => {
  const { title, content, startDate, endDate, price, note, dept, name, file, corporation, memo } = product;
  const date = new Date()
     ...
     <div>
       ...
       <div className='border p-2 rounded-lg border-gray-300'>
         <div className='flex flex-col'>
           ...
           {memo && ( // memo가 비어있지 않은 경우에만 보여줌
              <div className='flex items-center mt-1'>
                <h1 className='font-bold mr-2 sm:text-md text-xm'>Memo : </h1>
                <p className='text-gray-600 text-sm sm:text-md'>{memo}</p>
              </div>
           )}
         </div>
       </div>
