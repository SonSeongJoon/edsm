...
import {HomeReportItemOutput} from "../HomeReportItemOutput";

const HomeReportShow = ({ product }) => {
	const {
		title,
		file,
		name,
			...
			<div className="flex flex-col text-sm">
				...
				<div className="flex flex-col font-bold w-full mt-5">
					...
					{items.map((item, idx) => (
						...
						>
							<HomeReportItemOutput item={item} idx={idx + 1} />
						</div>
					))}
				</div>
			</div>
